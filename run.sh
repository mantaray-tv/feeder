...
while true; do
    ...
    # (npm list -g peerflix || npm i -g peerflix) && 
...
# torsocks peerflix $1 --vlc -- -I dummy --sout "'#transcode{vcodec=h264,acodec=aac,soverlay,vb=6000,ab=320}:std{access=http,mux=ts,dst=:8000/stream.ts}'" &
torsocks vlc /test.torrent -I dummy --sout '#transcode{vcodec=h264,acodec=aac,soverlay,vb=6000,ab=320}:std{access=http,mux=ts,dst=:8000/stream.ts}' &
while true; do
